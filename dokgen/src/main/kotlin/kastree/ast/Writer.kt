...
open class Writer(
    ...
    protected val elemsSinceLastLine = mutableListOf<Node>()

    protected fun endLine() = also {
        val elems = elemsSinceLastLine.toList() // a copy
        elemsSinceLastLine.clear()
    ...
    override fun visit(v: Node?, parent: Node) {
        ...
        v?.writeExtrasAfter()
        v?.also { elemsSinceLastLine.add(it) }
    }
        ...
        extras.forEach {
            when (it) {
                ...
                is Node.Extra.Comment -> {
                    if (it.startsLine && it.endsLine) {
                        lineEnd(it.text).lineBegin()
                    } else {
    ...
    protected fun stmtHasModifierLocalVarDeclAmbiguity(v: Node?, next: Node?): Boolean {
        ...
        val name = v.expr.name.lowercase()

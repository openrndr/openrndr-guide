...
fun main() {
    @Text """# Concurrency and multi-threading"""
    @Text """Here we talk about OPENRNDR's primitives for concurrency."""

    @Text """The largest complication in multi-threading in OPENRNDR lies in how the underlying graphics API (OpenGL)
...
performed on the primary draw thread or specially created draw threads. 
    ...
    @Text """## Coroutines"""
    @Text """Coroutines, as they are discussed here are a Kotlin specific framework for concurrency. Please read the 
...
an introduction to coroutines"""
    @Text """`Program` comes with its own coroutine dispatcher, which guarantees that coroutines will be handled on the 
...
coroutine does _not_ block the primary draw thread."""
    ...
    @Text """You may be asking, what is the purpose of the `Program` dispatcher if running coroutines blocks the primary
...
waiting for (off-thread) coroutines to complete and using the results to write to graphics resources."""
    ...
    @Text """In the below example we nest coroutines; the outer one is launched on the `Program` dispatcher, the inner  
 ...
 we can write the results to a graphics resource on the primary draw thread.
    ...
    @Text """## Secondary draw threads"""
    @Text """In some scenarios you may want to have a separate thread on which all graphic resources can be used and
drawing is allowed. In those cases you use `drawThread`."""

    @Text """Most graphic resources can be used and shared between threads, with the exception of the `RenderTarget`."""

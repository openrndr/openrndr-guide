...
The `ColorBufferLoader` loads URL strings, it supports loading from file and http/https."""
    ...
    @Text """Here we see that `fromUrl` can safely be called many times as `ColorBufferLoader` caches these requests.
Accessing `proxy.colorBuffer` touches its internal timestamp and queues the image for loading if the proxy state is
`NOT_LOADED`."""
    @Text """## Handling errors"""
    @Text """Sometimes loading fails; for example because the file was not found, or the http connection timed out.
...
should only occur if the loading of the image causes a non-IOException to be thrown. """
    ...
    @Text """## Cancelling a queued image"""
    @Text """Cancelling an image which is queued (and is thus in `QUEUED` state) for loading is done by calling

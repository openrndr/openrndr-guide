...
fun main() {
    ...
    @Code.Block
    run {
        val v2 = Vector2.uniforms(100, Vector2(0.0), Vector2(640.0))
        val v3 = Vector3.uniforms(100, Vector3(0.0), Vector3(640.0))
    }
    ...
    run {
        // Boolean
        val b = Boolean.random(probability = 0.2)

        // Int
        val i1 = Int.uniform(0, 640)

        // Double
        val d2 = Double.uniform(0.0, 640.0)
    ...
    `perlin()` and `value()` accept 2D and 3D arguments, `simplex()` up to 4D.
    
    All three method take an integer seed as first argument and return a `Double`.
    ...
    run {
        ...
        val d1 = perlin(1234, 0.1, 0.2)
        val d2 = perlin(1248, v2)
        val d3 = value(1234, 0.1, 0.2, 0.3)
        val d4 = simplex(1234, v4)
    }
    ...
    @Code.Block
    run {
        val v2 = Vector2.uniformRing(innerRadius = 0.0, outerRadius = 300.0)
            ...
            extend {
                ...
                for (y in 16 until height step 32) {
                    for (x in 16 until width step 32) {
                        val radius = perlinLinear(123, x * scale, y * scale) * 16.0 + 16.0
    ...
    application {
        ...
        program {
            extend {
                ...
                for (y in 16 until height step 32) {
                    for (x in 16 until width step 32) {
                        val radius = valueLinear(123, x * scale, y * scale) * 16.0 + 16.0
                        drawer.circle(x * 1.0, y * 1.0, radius)
                    }
                }
            }

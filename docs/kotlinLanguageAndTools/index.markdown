---
# File generated by dokgen. Do not edit. 
# Edit 'src/main/kotlin/docs/10_Kotlin_lang_and_tools/index.kt' instead.
layout: default
title: Kotlin language and tools
parent: ~
last_modified_at: 2025.02.09 22:17:21 +0000
nav_order: 1020
has_children: true
---
 
# Kotlin language and tools

OPENRNDR is written using the Kotlin programming language,
which is similar to Java but more modern and less verbose.

The Gradle build tool is used by the OPENRNDR projects 
to download dependencies, build and distribute projects.
 

[edit on GitHub](https://github.com/openrndr/openrndr-guide/blob/main/src/main/kotlin/docs/10_Kotlin_lang_and_tools/index.kt){: .btn .btn-github }